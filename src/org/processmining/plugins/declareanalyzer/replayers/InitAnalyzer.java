...
public class InitAnalyzer implements DeclareReplayer{
	...
	public InitAnalyzer(List<List<String>> dispositions){
	...
	public void process(int eventIndex, String event, XTrace trace, String traceId, DataSnapshotListener listener) {
		...
		for(String param : activationsPerTrace.keySet()){
			...
			if(eventIndex==0 && (!event.equals(mappingConstrParam.get(param)) || !isActivation)){
	...
	public Set<Integer> getBInitViolations(String traceId, String param1, String param2) {
		Set<Integer> output = new HashSet<Integer>();
		if(violations.get(traceId).get(param1).size() == 1  &&  violations.get(traceId).get(param2).size()  == 1){
	...
	public Set<Integer> getBInitFulfillments(String traceId, String param1, String param2) {
		Set<Integer> output = new HashSet<Integer>();
		if(fulfillments.get(traceId).get(param1).size() == 1  ||  fulfillments.get(traceId).get(param2).size()  == 1){
			output.addAll(fulfillments.get(traceId).get(param1));
			output.addAll(fulfillments.get(traceId).get(param2));
			return output;
		}
		return output;
	}
